...
struct SectionRange {
    ...
    bool overlap(const SectionRange& other) {
        return (other.begin_ >= begin_ && other.begin_ <= end_) ||
            (other.end_ >= begin_ && other.end_ <= end_);
    }
};
...
int main(int argc, char** argv) {
    ...
    for (auto p : data) {
        ...
        if (rng1.overlap(rng2) || rng2.overlap(rng1))
            overlap_counter++;
    }

    std::cout << "Number of pairs that overlaps: "
              << overlap_counter << std::endl;
